...
open class ConsoleLogger: Logger {
    ...
    open func logError(_ error: NSError, withMessage message: String) {
        print("\(message): \(error)")
    }
    
    open func session(_ session: Session, didReceiveResult result: Result) {
        print("")
        print("Session did receive response:  \(String(describing: result.HTTPSTatusCode)), \(String(describing: result.URL))")
